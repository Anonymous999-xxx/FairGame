...
def reshape_result_list(input_list, l1_num, l2_num, l3_num, l4_num, repeated_num):
    ...
    if len(input_list) != l1_num * l2_num * l3_num * l4_num * repeated_num:
        raise ValueError("输入列表的长度与预期的5维结构不匹配")
...
async def DGC_evaluate(
    ...
    # llm_config = config_list_from_json(env_or_file="./configure_list_gpt4o_kun.json", filter_dict=filter_dict)[0]
            ...
            for j in range(len(diplomatic_prompts)):
                for k in range(len(diplomatic_options)):
                    ...
                    for r in range(repeated_num):
                        ...
                        # 创建异步任务信息：
                        task_params = {  # 存储任务的参数
                            "task_id": gn * (len(versus_list) * len(diplomatic_prompts) * len(diplomatic_options) * repeated_num) 
    ...
    for b in range(0, len(tasks), batch_size * (repeated_num + redundancy)):
        ...
        json_dump_path = './record/Diplomatic_Game_Choices_unreal_gpt4o_raw-{}-temp.json'.format(evaluated_language)
...
async def multi_evaluate(
    ...
    for test_language in tqdm(evaluate_languages, desc="Outer Loop"): # with different language
        ...
        json_dump_path = './record/Diplomatic_Game_Choices_unreal_gpt4o_raw-{}-temp.json'.format(test_language)
...
if __name__ == '__main__':
    ...
    # deepseek-v3
    game_name = read_json('./DGC/DGC_game_name-unreal.json')
    max_num = 1
    game_name = {
    ...
    country = read_json('./DGC/DGC_country-unreal.json')
    country_num = 10 # 39
    country = {
    ...
    total_task_info = asyncio.run(
        multi_evaluate(
            ...
            batch_size = 50,                          # Number of tasks in one batch, related to LLM API access limitation
            TPM_sleep = 5,                            # Retry how many times when TPM or RPM is exceeded
        )
    )
    
    json_dump_path = './record/Diplomatic_Game_Choices_unreal_gpt4o_raw-final.json'
    write_to_json(total_task_info, json_dump_path)
