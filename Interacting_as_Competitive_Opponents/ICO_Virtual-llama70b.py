...
async def DGC_evaluate(
    ...
    for b in range(0, len(tasks), batch_size * (repeated_num + redundancy)):
        ...
        json_dump_path = './record/Diplomatic_Game_Choices_unreal_llama31_70b_raw-{}-temp.json'.format(evaluated_language)
...
async def multi_evaluate(
    ...
    for test_language in tqdm(evaluate_languages, desc="Outer Loop"): # with different language
        ...
        json_dump_path = './record/Diplomatic_Game_Choices_unreal_llama31_70b_raw-{}-temp.json'.format(test_language)
...
if __name__ == '__main__':
    ...
    total_task_info = asyncio.run(
        multi_evaluate(
            ...
            model = "meta-llama/Meta-Llama-3.1-70B-Instruct",                  # Model name
            ...
            batch_size = 60,                          # Number of tasks in one batch, related to LLM API access limitation
            TPM_sleep = 5,                            # Retry how many times when TPM or RPM is exceeded
        )
    )
    
    json_dump_path = './record/Diplomatic_Game_Choices_unreal_llama31_70b_raw.json'
    write_to_json(total_task_info, json_dump_path)
