...
async def DGC_evaluate(
    ...
    llm_config = config_list_from_json(env_or_file="./configure_list_qf-2.json", filter_dict=filter_dict)[0]
    ...
    for b in range(0, len(tasks), batch_size * (repeated_num + redundancy)):
        ...
        json_dump_path = './record/Diplomatic_Game_Choices_unreal_deepseekv3_raw-{}-temp-qf-2.json'.format(evaluated_language)
...
async def multi_evaluate(
    ...
    for test_language in tqdm(evaluate_languages, desc="Outer Loop"): # with different language
        ...
        json_dump_path = './record/Diplomatic_Game_Choices_unreal_deepseekv3_raw-{}-temp-qf-2.json'.format(test_language)
...
if __name__ == '__main__':
    ...
    total_task_info = asyncio.run(
        multi_evaluate(
            ...
            batch_size = 8,                          # Number of tasks in one batch, related to LLM API access limitation
            TPM_sleep = 5,                            # Retry how many times when TPM or RPM is exceeded
        )
    )
    
    json_dump_path = './record/Diplomatic_Game_Choices_unreal_deepseek_v3_raw-qf-2.json'
    write_to_json(total_task_info, json_dump_path)
