...
async def multi_evaluate(
    ...
    total_task_info = []
    evaluated_choise = ["flavor_N_location"] # ["flavor, location"]
    ...
    for choise in tqdm(evaluated_choise, desc="Outer Loop"):   # with different Observation Spaces
        for test_language in tqdm(evaluate_languages, desc="Inner Loop", leave=False): # with different language
            ...
            temp_task_info = await GGS_evaluate(
                ...
                eval_selection = choise,               # Evaluate what, "flavor" or "location" or "flavor_N_location"
...
if __name__ == '__main__':
    ...
    if args.data_type == "real":
        item = read_json('./data/item_LLM_wine-Real.json')
        item_type = "wine"
        item = item[item_type]

        flavor = read_json('./data/flavor_LLM-Real.json')
        max_num = 1 # total num: 8
        flavor = {
        ...
        location = read_json('./data/location_wine-Real.json')
        max_num = 1 # total num: 6
        location = {
        ...
        raise ValueError("This program is designed for real data only. You should change the data type to 'real'.")  
