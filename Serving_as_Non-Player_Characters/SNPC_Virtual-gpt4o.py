...
async def FGD_evaluate(
    ...
    tasks = []
    print("len(game_name) = {}".format(len(game_name)))
    print("len(race) = {}".format(len(race)))
    print("len(career) = {}".format(len(career)))
    print("repeated_num = {}".format(repeated_num))
    ...
    for b in range(0, len(tasks), batch_size * (repeated_num + redundancy)):
        ...
        json_dump_path = './record/Fantasy_Game_Discounts_unreal_gpt4o_raw-{}-temp.json'.format(evaluated_language)
...
async def multi_evaluate(
    ...
    for test_language in tqdm(evaluate_languages, desc="Loop", leave=False): # with different language
        ...
        json_dump_path = './record/Fantasy_Game_Discounts_unreal_gpt4o_raw-{}-temp.json'.format(test_language)
...
if __name__ == '__main__':
    ...
    json_dump_path = './record/Fantasy_Game_Discounts_unreal_gpt4o_raw-final.json'
    write_to_json(total_task_info, json_dump_path)
