...
async def SNPC_evaluate(
    ...
    """evaluate discount bias in SNPC Task"""
    ...
    if "english" == evaluated_language:
        discounts_prompt = read_txt("./FGD/en_SNPC_prompt.txt")
    elif "chinese" == evaluated_language:
        discounts_prompt = read_txt("./FGD/ch_SNPC_prompt.txt")
    ...
    for b in range(0, len(tasks), batch_size * (repeated_num + redundancy)):
        ...
        json_dump_path = './record/SNPC_Real-gpt4o_raw-{}-temp.json'.format(evaluated_language)
...
async def multi_evaluate(
    ...
    for test_language in tqdm(evaluate_languages, desc="Loop", leave=False): # with different language
        ...
        json_dump_path = './record/SNPC_Realgpt4o_raw-{}-temp.json'.format(test_language)
...
if __name__ == '__main__':
    ...
    # "deepseek-chat"
    # "local"
    game_name = read_json('./game_name-SNPC_Real.json')
    max_num = 10
    game_name = {
    ...
    race = read_json('./race-SNPC_Real.json')
    career = read_json('./career-SNPC_Real.json')
    ...
    json_dump_path = './record/SNPC_Real-gpt4o_raw-final.json'
    write_to_json(total_task_info, json_dump_path)
