...
async def SNPC_evaluate(
    ...
    if "english" == evaluated_language:
        discounts_prompt = read_txt("./en_SNPC_prompt.txt")
    elif "chinese" == evaluated_language:
        discounts_prompt = read_txt("./ch_SNPC_prompt.txt")
    ...
    for b in range(0, len(tasks), batch_size * (repeated_num + redundancy)):
        ...
        json_dump_path = './record/SNPC_Real-deepseekv3_raw-{}-temp.json'.format(evaluated_language)
...
async def multi_evaluate(
    ...
    for test_language in tqdm(evaluate_languages, desc="Loop", leave=False): # with different language
        ...
        json_dump_path = './record/SNPC_Real-deepseekv3_raw-{}-temp.json'.format(test_language)
...
if __name__ == '__main__':
    ...
    total_task_info = asyncio.run(
        multi_evaluate(
            ...
            model = api_model_name[model_name][0],    # Model name for API
    ...
    json_dump_path = './record/SNPC_Real-deepseekv3_raw-final.json'
    write_to_json(total_task_info, json_dump_path)
